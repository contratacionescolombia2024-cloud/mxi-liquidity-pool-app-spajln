...
export default function RetirosScreen() {
  ...
  // 🔥 DRASTIC FIX: Force re-render key with timestamp
  ...
  // 🔥🔥🔥 DRASTIC FIX: HARDCODED TRC20 TEXT - NO TRANSLATIONS
  const TRC20_NETWORK_TEXT = {
    es: 'Los retiros se procesarán en USDT por la red TRC20 (Tron)',
    en: 'Withdrawals will be processed in USDT via TRC20 network (Tron)',
    pt: 'As retiradas serão processadas em USDT pela rede TRC20 (Tron)'
  };

  const TRC20_WALLET_LABEL = {
    es: 'Dirección de Billetera USDT (TRC20)',
    en: 'USDT Wallet Address (TRC20)',
    pt: 'Endereço da Carteira USDT (TRC20)'
  };

  const TRC20_WALLET_PLACEHOLDER = {
    es: 'Ingresa tu dirección de billetera USDT TRC20 (Tron)',
    en: 'Enter your USDT TRC20 wallet address (Tron)',
    pt: 'Digite seu endereço de carteira USDT TRC20 (Tron)'
  };

  const TRC20_WARNING = {
    es: 'Solo direcciones TRC20 (Tron). No envíes a direcciones ETH.',
    en: 'Only TRC20 (Tron) addresses. Do not send to ETH addresses.',
    pt: 'Apenas endereços TRC20 (Tron). Não envie para endereços ETH.'
  };

  const currentLocale = t('locale') as 'es' | 'en' | 'pt';
  const networkText = TRC20_NETWORK_TEXT[currentLocale] || TRC20_NETWORK_TEXT.es;
  const walletAddressLabel = TRC20_WALLET_LABEL[currentLocale] || TRC20_WALLET_LABEL.es;
  const walletAddressPlaceholder = TRC20_WALLET_PLACEHOLDER[currentLocale] || TRC20_WALLET_PLACEHOLDER.es;
  const trc20Warning = TRC20_WARNING[currentLocale] || TRC20_WARNING.es;

  console.log('🔥🔥🔥 RETIROS TRC20 DEBUG:', {
    currentLocale,
    networkText,
    walletAddressLabel,
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {isTypeAvailable(selectedType) && (
            <View style={styles.card}>
              ...
              {/* 🔥🔥🔥 DRASTIC FIX: HARDCODED TRC20 BANNER */}
              <View style={styles.networkBanner}>
                <View style={styles.trc20IconContainer}>
                  <Text style={styles.trc20IconText}>TRC20</Text>
                </View>
              ...
              <View style={styles.inputContainer}>
                {/* 🔥🔥🔥 DRASTIC FIX: HARDCODED TRC20 LABEL */}
                ...
                <TextInput
                  ...
                  multiline
                />
                {/* 🔥🔥🔥 DRASTIC FIX: HARDCODED TRC20 WARNING */}
          ...
          <View style={[styles.card, styles.infoCard]}>
            <Text style={styles.infoTitle}>{t('importEntiendo que el problema del TRC20 persiste a pesar de los intentos anteriores. Voy a tomar un **abordaje drástico** para resolver esto de una vez por todas.

## Análisis del Problema

Revisando el código de `retiros.tsx`, veo que:
1. Las traducciones TRC20 están definidas en `i18n.ts`
2. El componente usa `useLanguage()` para obtener las traducciones
3. Ya hay elementos visuales (banners, badges) para TRC20
4. Hay un `renderKey` para forzar re-render

El problema probablemente es que las traducciones no se están cargando correctamente o hay un problema de caché más profundo.

## Solución Drástica

Voy a:
1. **Eliminar completamente** cualquier referencia a ETH/Ethereum
2. **Hardcodear** el texto TRC20 directamente en el componente (sin depender de traducciones)
3. **Simplificar** el sistema de traducciones para TRC20
4. **Forzar** una actualización visual más agresiva

<write file="app/(tabs)/(home)/retiros.tsx">
import React, { useState, useEffect } from 'react';
import {
  View,
  Text,
