
# TÉRMINOS Y CONDICIONES DE USO - POOL DE LIQUIDEZ MAXCOIN (MXI)

**Última actualización: 15 de Enero de 2025**

## 1. ACEPTACIÓN DE LOS TÉRMINOS

Al registrarse y utilizar la aplicación Pool de Liquidez Maxcoin (en adelante, "la Aplicación"), usted acepta estar legalmente vinculado por estos Términos y Condiciones. Si no está de acuerdo con alguna parte de estos términos, no debe utilizar la Aplicación.

## 2. DESCRIPCIÓN DEL SERVICIO

La Aplicación permite a los usuarios participar en un pool de liquidez para la criptomoneda Maxcoin (MXI) mediante:

- Registro de usuario con información personal verificable
- Compra de tokens MXI mediante pagos en USDT a través de Binance
- Participación en un sistema de referidos multinivel
- Generación de rendimientos (yield) sobre tokens MXI adquiridos
- Retiro de comisiones y tokens MXI según las condiciones establecidas

## 3. ELEGIBILIDAD Y REGISTRO

### 3.1 Requisitos de Elegibilidad
- Ser mayor de 18 años
- Tener capacidad legal para celebrar contratos vinculantes
- No estar ubicado en jurisdicciones donde las criptomonedas estén prohibidas
- Proporcionar información veraz y verificable

### 3.2 Proceso de Registro
- Solo se permite una cuenta por persona
- Se requiere verificación de identidad (KYC) para retiros
- La información proporcionada debe coincidir con documentos oficiales
- El número de identificación será verificado y no puede duplicarse

## 4. INVERSIÓN Y COMPRA DE TOKENS

### 4.1 Montos de Inversión
- Inversión mínima: 50 USDT
- Inversión máxima por cliente: 100,000 USDT
- Los pagos se realizan exclusivamente en USDT a través de Binance
- La cantidad de tokens MXI recibidos depende del precio de la fase actual

### 4.2 Fases de Preventa
La preventa se divide en tres fases con precios progresivos:
- Fase 1: Precio inicial
- Fase 2: Precio incrementado
- Fase 3: Precio final de preventa

### 4.3 Proceso de Pago
- Los pagos deben realizarse a la dirección de billetera Binance proporcionada
- Se debe proporcionar el ID de transacción (TxID) de Binance
- Los pagos pueden ser verificados automáticamente o requerir aprobación manual
- Los pagos expiran después de 30 minutos si no se completan

## 5. SISTEMA DE REFERIDOS

### 5.1 Estructura de Comisiones
- Nivel 1: 3% de las compras de referidos directos
- Nivel 2: 2% de las compras de referidos de segundo nivel
- Nivel 3: 1% de las compras de referidos de tercer nivel

### 5.2 Requisitos para Retiro de Comisiones
- Mínimo 5 referidos activos de primer nivel
- 10 días desde la fecha de registro
- Verificación KYC aprobada
- Los referidos deben haber realizado al menos una compra de MXI

### 5.3 Ciclo de Liberación
Las comisiones están disponibles para retiro una vez cumplidos los requisitos mencionados.

## 6. SISTEMA DE VESTING Y GENERACIÓN DE RENDIMIENTOS

### 6.1 Vesting (Minería)
- Los tokens MXI comprados directamente generan rendimientos del 0.005% por hora
- Los tokens MXI obtenidos de comisiones unificadas también generan rendimientos
- Los rendimientos generados NO aumentan el porcentaje de vesting
- Se requieren 10 referidos activos para unificar el saldo de vesting al balance principal

### 6.2 Cálculo de Rendimientos
Los rendimientos se calculan en tiempo real basándose en:
- Cantidad de MXI en vesting
- Tasa de rendimiento por minuto
- Tiempo transcurrido desde la última actualización

## 7. RETIROS

### 7.1 Retiro de Comisiones (USDT)
Requisitos:
- 5 referidos activos de primer nivel
- 10 días de membresía
- KYC verificado y aprobado
- Dirección de billetera USDT válida

### 7.2 Retiro de Tokens MXI
Requisitos:
- 5 referidos activos de primer nivel
- KYC verificado y aprobado
- Pool de liquidez lanzado oficialmente
- Sistema de liberación por fases (10% inicial, 10% cada 7 días)

### 7.3 Sistema de Liberación por Fases
Para proteger el valor del token y prevenir caídas de precio:
- 10% de los tokens disponibles al lanzamiento
- 10% adicional cada 7 días
- Liberación completa en aproximadamente 10 semanas

## 8. VERIFICACIÓN KYC (KNOW YOUR CUSTOMER)

### 8.1 Requisitos KYC
- Documento de identidad válido (pasaporte, cédula o licencia de conducir)
- Fotografía del frente y reverso del documento
- Selfie con el documento (si se requiere)
- Información personal verificable

### 8.2 Proceso de Verificación
- Envío de documentos a través de la Aplicación
- Revisión por el equipo administrativo (24-48 horas)
- Notificación de aprobación o rechazo
- Posibilidad de reenvío en caso de rechazo

### 8.3 Protección de Datos
- Todos los documentos se almacenan de forma encriptada
- Cumplimiento con regulaciones internacionales de protección de datos
- No se comparte información con terceros sin consentimiento

## 9. CIERRE DEL POOL Y LANZAMIENTO

### 9.1 Fecha de Cierre
- Fecha límite: 15 de enero de 2025 a las 12:00 UTC
- No se aceptarán nuevos participantes después de esta fecha
- Objetivo: 250,000 participantes

### 9.2 Lanzamiento Oficial
- Los fondos se derivarán al pool de liquidez
- Se entregará valor a la moneda MXI
- Inicio de operaciones en exchanges
- Activación del sistema de retiros por fases

## 10. RIESGOS Y ADVERTENCIAS

### 10.1 Riesgos de Inversión
ADVERTENCIA: La inversión en criptomonedas conlleva riesgos significativos:
- Volatilidad extrema de precios
- Posibilidad de pérdida total de la inversión
- Riesgos regulatorios y legales
- Riesgos tecnológicos y de seguridad

### 10.2 No Garantías
- No se garantiza ningún rendimiento específico
- El valor de MXI puede fluctuar significativamente
- Los rendimientos pasados no garantizan resultados futuros
- La liquidez del token depende de condiciones de mercado

### 10.3 Responsabilidad del Usuario
El usuario es responsable de:
- Mantener seguras sus credenciales de acceso
- Verificar todas las transacciones antes de confirmarlas
- Comprender los riesgos asociados con criptomonedas
- Cumplir con las leyes fiscales de su jurisdicción

## 11. PROHIBICIONES Y CONDUCTA DEL USUARIO

### 11.1 Actividades Prohibidas
Está estrictamente prohibido:
- Crear múltiples cuentas
- Proporcionar información falsa o fraudulenta
- Manipular el sistema de referidos
- Realizar actividades de lavado de dinero
- Utilizar la Aplicación para fines ilegales
- Intentar hackear o comprometer la seguridad

### 11.2 Consecuencias
La violación de estos términos puede resultar en:
- Suspensión o terminación de la cuenta
- Pérdida de fondos y comisiones
- Reporte a autoridades competentes
- Acciones legales

## 12. MODIFICACIONES Y ACTUALIZACIONES

### 12.1 Cambios en los Términos
Nos reservamos el derecho de modificar estos términos en cualquier momento:
- Los cambios se notificarán a través de la Aplicación
- El uso continuado implica aceptación de los nuevos términos
- Los cambios significativos requerirán aceptación explícita

### 12.2 Actualizaciones de la Aplicación
- La Aplicación puede actualizarse periódicamente
- Algunas actualizaciones pueden ser obligatorias
- Las funcionalidades pueden cambiar o eliminarse

## 13. PROPIEDAD INTELECTUAL

### 13.1 Derechos de Autor
- Todo el contenido de la Aplicación está protegido por derechos de autor
- El logo, nombre y marca MXI son propiedad exclusiva
- No se permite el uso no autorizado de materiales

### 13.2 Licencia de Uso
Se otorga una licencia limitada, no exclusiva e intransferible para:
- Usar la Aplicación según estos términos
- Acceder a su cuenta personal
- Participar en el pool de liquidez

## 14. LIMITACIÓN DE RESPONSABILIDAD

### 14.1 Exclusión de Garantías
LA APLICACIÓN SE PROPORCIONA "TAL CUAL" SIN GARANTÍAS DE NINGÚN TIPO:
- No garantizamos disponibilidad ininterrumpida
- No garantizamos ausencia de errores
- No garantizamos resultados específicos

### 14.2 Limitación de Daños
En ningún caso seremos responsables por:
- Pérdidas de ganancias o ingresos
- Pérdida de datos o información
- Daños indirectos, incidentales o consecuentes
- Daños que excedan el monto invertido

## 15. RESOLUCIÓN DE DISPUTAS

### 15.1 Proceso de Resolución
En caso de disputa:
1. Contactar al soporte técnico
2. Intentar resolución amistosa
3. Mediación si es necesario
4. Arbitraje vinculante como último recurso

### 15.2 Jurisdicción
Estos términos se rigen por las leyes de [JURISDICCIÓN A ESPECIFICAR].

## 16. TERMINACIÓN

### 16.1 Terminación por el Usuario
El usuario puede cerrar su cuenta en cualquier momento:
- Sujeto a completar retiros pendientes
- Cumplir con obligaciones pendientes
- Pérdida de acceso a fondos no retirados

### 16.2 Terminación por la Plataforma
Podemos terminar cuentas por:
- Violación de estos términos
- Actividad fraudulenta o sospechosa
- Inactividad prolongada
- Razones legales o regulatorias

## 17. DISPOSICIONES GENERALES

### 17.1 Acuerdo Completo
Estos términos constituyen el acuerdo completo entre las partes.

### 17.2 Divisibilidad
Si alguna disposición es inválida, las demás permanecen en vigor.

### 17.3 Renuncia
La falta de ejercicio de un derecho no constituye renuncia al mismo.

### 17.4 Cesión
No puede ceder sus derechos sin nuestro consentimiento previo.

## 18. CONTACTO Y SOPORTE

Para preguntas o soporte:
- Utilice la función de soporte dentro de la Aplicación
- Tiempo de respuesta: 24-48 horas
- Soporte disponible en español e inglés

## 19. ACEPTACIÓN

AL REGISTRARSE EN LA APLICACIÓN, USTED CONFIRMA QUE:
- Ha leído y comprendido estos Términos y Condiciones
- Acepta estar legalmente vinculado por ellos
- Comprende los riesgos asociados con la inversión en criptomonedas
- Proporciona información veraz y verificable
- Cumple con todos los requisitos de elegibilidad

---

**IMPORTANTE**: Estos términos y condiciones son legalmente vinculantes. Si no está de acuerdo con alguna parte, no debe utilizar la Aplicación. Se recomienda consultar con un asesor legal o financiero antes de realizar inversiones en criptomonedas.

**Fecha de vigencia**: 15 de Enero de 2025
**Versión**: 1.0
